...
<div x-data="{ open:false, title:'', chapter_number:'', release_date:'', init(){
    ...
    const rows = {{ count($chapters) }};
    this.chapter_number = rows + 1;
}, reset(){ this.title=''; this.release_date=''; this.init(); } }">
    <!-- Page Heading -->
    <div class="container mx-auto px-4 pt-6 lg:px-8 lg:pt-8">
        <div class="flex flex-col gap-2 text-center sm:flex-row sm:items-center sm:justify-between sm:text-start">
            <div class="grow">
                <h1 class="mb-1 text-xl font-bold text-zinc-500">Chapters of {{ $manga->title }}</h1>
    ...
    <div x-cloak x-show="open" class="fixed inset-0 z-50 flex items-center justify-center" @keydown.escape.window="open=false">
        ...
        <div class="relative z-10 w-full max-w-xl rounded-lg bg-white text-zinc-900 shadow-xl">
            ...
            <form action="{{ route('manga.chapters.store', $manga->id) }}" method="POST" class="space-y-4 p-5">
                @csrf
                <div>
                                ...
                                <tr class="border-b border-zinc-100 hover:bg-zinc-50">
                                    ...
                                    <td class="p-3 text-start">
                                        <div class="flex items-center space-x-2">
                                            <a href="#" class="text-blue-500 hover:underline" title="Edit Chapter">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                                    <path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" />
                                                    <path fill-rule="evenodd" d="M5 13.828V15h1.172l7.586-7.586-1.172-1.172L5 13.828z" clip-rule="evenodd" />
                                                </svg>
                                            </a>
                                            <form action="#" method="POST" class="inline" title="Delete Chapter">
                                                @csrf
                                                @method('DELETE')
                                            ...
                                            <a href="{{ route('chapter.pages', $chapter->id) }}" class="text-green-500 hover:underline" title="View Pages">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
